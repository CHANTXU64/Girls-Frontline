...
function Tab_Timetable_AddNewTimePoint() {
    ...
    switch(true) {
        ...
        case total_time >= Tab_Timetable_getMaxTime():
            ...
            break;
        case tab_Timetable_time.indexOf(total_time) != -1:
            alert("已经添加过这个收取时间点");
            break;
        default:
...
function Tab_Timetable_AddNewTooltip(time, position) {
    ...
    if (tab_Timetable_time.indexOf(time) % 2 == 0) {
...
function Tab_Timetable_DeleteThisTimePoint(time) {
    tab_Timetable_time.remove(time);
    ...
    if (tab_Timetable_time.length == 0) Tab_Timetable_InputTotalTime_enable();
}
...
function Tab_Timetable_DeleteAllTimePoints() {
    var times = tab_Timetable_time.length;
    for (var i = 0; i < times; i++) {
        Tab_Timetable_DeleteThisTimePoint(tab_Timetable_time[0]);
    }
